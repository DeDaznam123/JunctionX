...
class MultiLabelRobertaWithConstraints(nn.Module):
    ...
    def __init__(self, model_name, num_labels=11, dropout=0.1):
        super().__init__()
    ...
    def compute_constrained_loss(self, logits, labels):
        """
        Compute loss with constraint enforcement.
        ...
        # Standard BCE loss for all labels
        ...
        primary_probs = torch.sigmoid(logits[:, 0])  # Probability of hate
        primary_labels = labels[:, 0]
        
        # For nothate samples (primary_label=0), secondary labels should be 0
        ...
        # Combined loss
        total_loss = total_loss + 0.5 * constraint_penalty

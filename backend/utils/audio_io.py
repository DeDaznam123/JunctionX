...
def _resolve_ffmpeg() -> str:
    ...
    if ff and Path(ff).exists():
        return ff
    # 2) bundled binary (optional, for hosting)
    repo = Path(__file__).resolve().parents[2]  # adjust if your depth differs
...
def url_to_wav(url: str, out_dir: Path, sr: int = 16000) -> Path:
    ...
    # DEBUG: print the command so we know which binary is used
